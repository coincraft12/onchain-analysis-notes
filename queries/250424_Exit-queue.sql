...
SELECT
    ...
    -- 3개 테이블 중 존재하는 시간 중 가장 앞에 있는 것을 기준으로 통합
    e.exit_queue,
    -- Exit 대기 중인 검증자 수
    s.eth_staked,
    -- 누적 스테이킹 ETH
...
FROM
    exit_data e FULL
    OUTER JOIN staked_data s ON e.time = s.time -- Exit ↔ Staking을 날짜 기준으로 병합
    FULL
    OUTER JOIN price_data p ON COALESCE(e.time, s.time) = p.time -- (e.time, s.time 중 먼저 존재하는 것과 price를 병합 → 전체 날짜 범위 보존)
ORDER BY
    time;
